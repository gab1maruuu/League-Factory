...
<body class="bg-zinc-950 text-white font-sans">
    ...
    <main class="container mx-auto px-4 py-8">
        <a href="index.php?action=my_teams" class="text-zinc-400 hover:text-white mb-4 inline-block">&larr; Volver a Mis
            Equipos</a>
            ...
            <div class="flex flex-col md:flex-row items-start gap-8">
                ...
                <div class="w-full md:w-1/3 text-center">
                    ...
                    <form action="index.php?action=update_team_photo" method="POST" enctype="multipart/form-data"
                        ...
                        <button type="submit"
                            class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-500 transition w-full text-sm">Guardar
                            Foto</button>
                    </form>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    <h2 class="text-2xl font-bold mb-6 text-indigo-400 border-b border-zinc-800 pb-2">Miembros del
                        Equipo</h2>

                    <!-- Search User to Add -->
                    <div class="mb-8 relative">
                        <label class="block text-sm font-bold text-zinc-400 mb-2">Añadir Miembro (Buscar por
                            usuario)</label>
                        <input type="text" id="user-search" placeholder="Escribe un nombre de usuario..."
                            class="w-full bg-zinc-950 border border-zinc-800 text-white rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-500 transition">
                    ...
                    <div class="space-y-4">
                        ...
                        <?php else: ?>
                            <?php foreach ($members as $member): ?>
                                <div
                                    class="flex items-center justify-between p-4 bg-zinc-950/50 rounded-lg border border-zinc-800">
                                    ...
                                    <?php if ($member['id'] != $_SESSION['user_id']): // Can't remove self (captain) ?>
                                        <form action="index.php?action=remove_member" method="POST"
                                            onsubmit="return confirm('¿Seguro que quieres eliminar a este miembro?');">
                                    ...
                                    <?php else: ?>
                                        <span
                                            class="text-xs text-indigo-500 font-bold border border-indigo-500/30 px-2 py-1 rounded">CAPITÁN</span>
                                    <?php endif; ?>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
